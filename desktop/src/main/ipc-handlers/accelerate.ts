...
export function accelerateHandlers(): void {
  ipcMain.handle('accelerate:inventory', async (_event, projectPath: string): Promise<string> => {
    return accelerate(['inventory', projectPath])
  })

  ipcMain.handle('accelerate:parse', async (_event, filePath: string): Promise<string> => {
    return accelerate(['parse', filePath])
  })

  ipcMain.handle('accelerate:run', (_event, filePath: string): Promise<string> => {
    return new Promise((resolve, reject) => {
      ...
      child.stdout.on('data', (chunk: Buffer) => {
        for (const line of chunk.toString().split('\n').filter(Boolean)) {
          _event.sender.send('accelerate:run-line', line)
        }
      })
      child.stderr.on('data', (chunk: Buffer) => {
        stderr += chunk.toString()
