...
graph {
    ...
    accelerator issue_triage {
        purpose = "阅读上面的 context 中所有 issue 数据。对每个 issue:1) 分类为 bug / feature / enhancement / question / docs / other;2) 评估优先级 P0-P3(P0=紧急阻塞, P1=重要, P2=正常, P3=低优先);3) 判断是否 stale(超过30天无更新);4) 标记可能重复的 issues。输出分类后的 issue 列表,按优先级排序。格式:优先级 | #编号 | 标题 | 分类 | 状态。不要修改任何文件。"
        models = ["deepseek-v4-flash"]
        policy = "captain"
        tools = []
    }
