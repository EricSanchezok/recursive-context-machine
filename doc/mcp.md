...
MCP (Model Context Protocol) 让 agent 能通过标准化的对外接口调用外部工具——搜索、查数据库、操作文件系统等。RCM 通过 `--mcp-server` 参数配置，支持 stdio 子进程和 Streamable HTTP 两种传输方式。
...
大多数 MCP 服务器在文档中展示的都是 Claude Desktop 的 JSON 配置格式。映射到 RCM 很简单：`command` + `args` 用空格拼起来就行。
