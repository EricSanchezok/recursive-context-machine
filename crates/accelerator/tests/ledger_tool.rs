...
#[tokio::test]
async fn concurrent_transitions_on_shared_run_dir_stay_consistent() {
    let (dir, env) = run_dir_env();
    let tool = std::sync::Arc::new(LedgerTool);

    call(
        &tool,
        serde_json::json!({"op": "add", "id": "shared", "title": "contended entry"}),
        &env,
    )
    .await
    .unwrap();
    call(
        &tool,
        serde_json::json!({"op": "start", "id": "shared"}),
        &env,
    )
    .await
    .unwrap();

    // Two racing completions: exactly one may win the running→completed
    // migration; the loser must receive an illegal-transition error, and
    // the persisted book must stay consistent with the winner.
    ...
    for _ in 0..2 {
        ...
        handles.push(tokio::spawn(async move {
            call(
                &tool,
                serde_json::json!({"op": "complete", "id": "shared", "result": "done"}),
                &env,
            )
            .await
        }));
    ...
    let outcomes: Vec<Result<String, String>> = joined
        .into_iter()
        .map(|outcome| outcome.expect("ledger task must not panic"))
        .collect();
    ...
    assert!(
        rejections[0].contains("illegal ledger transition"),
        "got: {}",
        rejections[0]
    );

    let digest = ledger_digest_for(dir.path()).expect("digest after writes");
    assert_eq!(digest.by_status.get("completed"), Some(&1));
}
